...
class ListSiswaWaliKelas extends Component
{
    ...
    {
        $siswa = Siswa::select('siswas.NISN', 'siswas.user', 'user_profiles.nama', 'kontrak_semesters.sakit', 'kontrak_semesters.izin', 'kontrak_semesters.alpa')->join('user_profiles', 'user_profiles.user', '=', 'siswas.user')->join('kontrak_semesters', 'kontrak_semesters.siswa', '=', 'siswas.nisn')->where('kelas', $this->kelas)->where('kontrak_semesters.status', 'ongoing')->get();
        return view('livewire.list-siswa-wali-kelas', compact('siswa'));
    }
